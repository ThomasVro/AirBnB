...
for month in range(1, 13):
    ...
    for listing_id in listing_ids:  # pour chaque annonce
        ...
        while j < len(tab_date_month):
            ...
            if temp_month1 == {}:
                month1 = get_items_fetchall("select * from "+constants.LISTINGS+" where listing_id='" +
...
for listing_id in listing_ids:
    ...
    if liste_periodes != [(datetime.date(year, month, 1), datetime.date(year, 12, calendar.monthrange(year, 12)[1]))] and liste_periodes != []:
        commentaires = get_items_fetchall("select * from "+constants.REVIEWS+" where listing_id='" +
    ...
    if liaisons != {}:
        ...
        print("liaisons_"+str(listing_id)+".json - Fait.")
